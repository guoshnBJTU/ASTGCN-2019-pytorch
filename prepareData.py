...
def search_data(sequence_length, num_of_depend, label_start_idx,
    ...
    if label_start_idx + num_for_predict > sequence_length: # 判断了未来值是否存在
        return None
...
def get_sample_indices(data_sequence, num_of_weeks, num_of_days, num_of_hours,
    ...
    '''
    week_sample, day_sample, hour_sample = None, None, None  # 如果任意一个依赖的长度是0，则该项为None，具有返回值

    # 判断是否存在label,不存在的话直接返回None
    if label_start_idx + num_for_predict > data_sequence.shape[0]:  # 判断了未来值是否存在
        return week_sample, day_sample, hour_sample, None

    if num_of_weeks > 0:  # 需要此类依赖
        week_indices = search_data(data_sequence.shape[0], num_of_weeks,
                                   label_start_idx, num_for_predict,
                                   7 * 24, points_per_hour)
        if not week_indices:  # 但是此类依赖不存在，所以该样本无法构造
    ...
    target = data_sequence[label_start_idx: label_start_idx + num_for_predict]  # 不写维度切片的话是按着第一个维度切片
...
def read_and_generate_dataset(graph_signal_matrix_filename,
    ...
    train_x = np.concatenate(training_set[:-2], axis=-1)  # (B,N,F,T'),把所有历史依赖都按着时间顺序串起来
...
def normalization(train, val, test):
    '''
    只利用train当中的最大值和最小值进行归一化,归一化到均值为0
    Parameters
    ----------
    ...
    mean = train.mean(axis=(0,1,3), keepdims=True) # the average of the array elements along (batch, nb_nodes, timestamp)
